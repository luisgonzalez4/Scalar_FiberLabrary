...
def SMF(A,w,beta2,beta3,gamma,L,step):
     ...
     D=np.exp((D2+D3)*h)
     hhz= 1j*gamma*h
     temp=A*np.exp(np.abs(A**2 )*hhz/2)
     for i in range(0,step):
        f_temp=np.fft.ifft(temp)*D
        A=np.fft.fft(f_temp)
        temp=A*np.exp(np.abs(A)**2 *hhz)
     A=temp*np.exp(-np.abs(A)**2 *hhz/2)
     return A
...
def DopedFibre(A,t,w,lambda0,BW,beta2,gamma,g0,Esat,L,step):
    ...
    for i in range(step):
        ...
        E = pls.Energy(temp, t)
        g_sat = g0 / (1 + E / Esat)  
        Linear = np.exp((D2 + 0.5 * g_sat * GainShape) * h)

        f_temp = np.fft.ifft(temp) * Linear
        A=np.fft.fft(f_temp)
        temp = A * np.exp(np.abs(A)**2 * hhz)
    A=temp*np.exp(-np.abs(A)**2 *hhz/2)
    return A
